...
- **排版调节**：字号、行距、页边距、5 种跨平台中文字体（宋体 / 黑体 / 楷体 / 圆体 / 仿宋）、三套主题（日间 / 护眼 / 夜间）
  - 页边距调的是**正文两侧留白**（0–120px），手机上同样生效；三套主题会一并接管侧栏与面板的配色，夜间不会留一块白面板
- **自定义字体**：可上传 `.ttf / .otf / .woff / .woff2`，跨平台一致生效
- ⚠️ **iPhone / iPad 上的字体说明**：iOS Safari 会屏蔽系统字体名，5 种内置字体在 iPhone 上会塌成两组（宋/楷/仿 一类、黑/圆 一类）。设置面板会把当前浏览器**真正不可用**的字体标灰并给「不可用」角标；想要确定的字形，请用「＋自定义」上传字体文件，走 FontFace 注册，各平台完全一致。
