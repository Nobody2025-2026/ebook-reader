...
// 读写的失败处置在 ./idb-guard —— 排版设置(settings.ts) 与自定义字体(customFont.ts)
// 也走同一套口径，别再各自 import idb-keyval 绕过守卫（绕过就是没兜底）。
// 这里 re-export 只为让调用方的 import 路径不变。
import { readGuard, writeGuard } from './idb-guard'
export { StorageWriteError, onStorageFailure, writeErrorText } from './idb-guard'
...
export async function saveBook(meta: BookMeta, file: Blob): Promise<void> {
  // 整本书几十 MB，是最容易撞上配额的一次写入 —— 失败必须让用户知道
  await writeGuard('导入书籍', async () => {
...
export async function saveProgress(id: string, progress: ReadingProgress): Promise<void> {
  await writeGuard('保存进度', () => set(KEY_PROGRESS + id, progress))
}
...
export async function clearProgress(id: string): Promise<void> {
  await writeGuard('清除进度', () => del(KEY_PROGRESS + id))
}
...
export async function addBookmark(bookId: string, bm: Bookmark): Promise<boolean> {
  ...
  await writeGuard('添加书签', () => set(KEY_BOOKMARKS + bookId, sortBookmarks([...list, bm])))
  return true
}

export async function removeBookmark(bookId: string, id: string): Promise<void> {
  const list = (await get<Bookmark[]>(KEY_BOOKMARKS + bookId)) ?? []
  await writeGuard('删除书签', () =>
    set(
...
export async function updateBookCover(
  ...
  if (!meta) return
  await writeGuard('更新封面', () => set(KEY_META + id, { ...meta, cover, coverVersion: version }))
}

/** 删书必须连带删进度、书签、阅读统计和高亮笔记，否则会留下孤儿记录 */
export async function deleteBook(id: string): Promise<void> {
  // 整体包一层：删到一半失败（书没了、笔记还在）比全失败更难解释，
  // 但至少抛出去让调用方说明白，而不是装作删干净了
  await writeGuard('删除书籍', async () => {
...
export async function addAnnotation(bookId: string, ann: Annotation): Promise<boolean> {
  ...
  if (dup) return false
  await writeGuard('添加高亮', () => set(KEY_ANNOTATIONS + bookId, [...list, ann]))
  return true
}
