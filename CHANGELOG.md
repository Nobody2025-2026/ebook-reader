...
- **字体不再"点了没反应却毫无解释"**
  - iPhone 的 Safari 出于字体指纹防护会屏蔽系统字体名，写 `font-family: "Kaiti SC"` 实际仍是默认字体——用户在设置里点半天看不到变化，只会以为软件坏了。
  - 新增运行时探测（`src/lib/fontAvailability.ts`，宽度比对法，两个回退字体交叉验证）：进阅读页时探一次内置字体在本机的真实可用性，**不可用的标灰 + 「不可用」角标**，并说明可以用「＋自定义」上传字体文件（走 FontFace 注册，各平台一致）。
  - 选中的字体恰好不可用时会额外给一条红字提示，明确"正文实际用的是浏览器替代字体"。
